program luc
    ...
    integer, dimension(0:31) :: intKey, intMsg
    integer :: decipher, i, l
    character(len=11) :: charMsg
    character(len=32) :: hexMsg
    
    ! Get key
    write(*,*) 'Enter key'
    read(*,formatInput) (intKey(i), i = 0, 31)

    ! Get message
    call readWord(charMsg)

    ! Convert message to hex and print it
    call word2hex(charMsg, hexMsg, l)
    call printHex(hexMsg, l)

    do i = 0, l
        read(hexMsg(i+1:i+1), '(z1)') intMsg(i)
    end do
    ...
    call expand(message, intMsg, 32)
    call expand(key, intKey, 32)
    ...
    write(*,*) 'Ciphertext'
    call compress(message, intMsg, 32)
    write(*, formatOutput) (intMsg(i), i = 0, 31)
    ...
    call compress(message, intMsg, 32)
    call compress(key, intKey, 32)

    ! Print compressed version of expanded key/message (originals)
    write(*,*) 'Decrypted key'
    write(*,formatOutput) (intKey(i), i = 0, 31)
